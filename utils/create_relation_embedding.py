...
logging.basicConfig(
    ...
    if not osp.exists(osp.join(path, kv_path)):
        ...
        model.train(
            stripped_relations, total_examples=len(stripped_relations), epochs=3
        )
        keyedVectors = {}
        keyedVectors_le = {}
        ...
        for i in range(len(stripped_relations)):
            relation_embedding = np.array(
                [model.wv[word] for word in stripped_relations[i]]
            ).mean(axis=0)
        ...
        save_word2vec_format(
            binary=True,
            fname=osp.join(path, kv_path),
            vocab=keyedVectors,
            vector_size=dimensions,
        )
        save_word2vec_format(
            binary=True,
            fname=osp.join(path, kv_path_le),
            vocab=keyedVectors_le,
            vector_size=dimensions,
        )
