...
def shuffle_dataset(dataset):
    df = pd.DataFrame(
        [dataset.edge_index[0], dataset.edge_index[1], dataset.edge_type]
    ).T
    df = df.sample(frac=1)
    edge_attributes = torch.tensor(list(df[2].values), dtype=torch.long)
    edge_index = torch.tensor(
        [list(df[0].values), list(df[1].values)], dtype=torch.long
    )
...
def load_data(dataset, hidden=64, node_embedding_size=16, embedding_path="embeddings"):
    ...
    cne.create_node_embedding(
        all_data, dataset, dimensions=node_embedding_size, workers=4
    )
    ...
    embedded_nodes = KeyedVectors.load_word2vec_format(
        "embeddings/node_embedding_" + dataset + "_" + str(node_embedding_size) + ".kv"
    )
    embedded_relations = KeyedVectors.load_word2vec_format(
        "embeddings/relation_embedding_le_" + dataset + "_" + str(hidden) + ".bin",
        binary=True,
    )
