...
def trainer(args, DATASET, train_loader, val_loader, test_loader, relation_embeddings, num_relations,
    ...
    tw.init_writer(DATASET, gap.args2string(args))
    ...
    for epoch in range(1, num_epoch+1):
        ...
        if epoch % epoch_per_val == 0: #TODO: undo break
            val_loss, val_node_acc, val_graph_acc, val_lp_auc, val_lp_ap = \
                train_transductive(model, optimizer, val_loader,
                        args.device,
                        node_multi_label,
                        graph_multi_label,
                        mode="val")
        ...
        if epoch % epochs_per_test == 0: #TODO undo break
            test_loss, test_node_acc, test_graph_acc, test_lp_auc, test_lp_ap = \
                train_transductive(model, optimizer, test_loader,
                        args.device,
                        node_multi_label,
                        graph_multi_label,
        ...
        if epoch % save_per_epoch == 0:
            torch.save(model, path.join(out_path, DATASET + ".pkl"))
